...
class CoreMarkProcessor(BaseProcessor):
    ...
    def _build_timeseries_object(self, time_series_data: List[Dict],
                                  run_number: int) -> tuple[Dict[str, TimeSeriesPoint], TimeSeriesSummary]:
        """
        ...
        for sequence, row in enumerate(time_series_data):
            iteration = row.get('iteration', sequence + 1)
